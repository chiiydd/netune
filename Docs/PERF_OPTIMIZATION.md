...
- **serde_json from_slice**: benchmark 比 from_str 慢 31%（UTF-8 验证开销），已改回 from_str
...
## Phase 3: 回归修复 (2026-05-19)

### 发现

Phase 1 将 `inner_request` 从 `resp.text()` + `from_str` 改为 `resp.bytes()` + `from_slice`，
声称避免了 String 分配。但 benchmark 显示 `from_slice` 比 `from_str` **慢 31%**：

| 方法 | 耗时 |
|------|------|
| `serde_json::from_str` | 426 µs |
| `serde_json::from_slice` | 559 µs |

原因：`from_slice` 需要在解析过程中验证 UTF-8，而 `from_str` 输入已经是合法 `&str` 可跳过验证。

### 修复

改回 `resp.text()` + `serde_json::from_str()`，API 响应反序列化性能恢复。
